 ...
 */
@Component
public class JaxBGpxReader {

    public JaxBGpxReader() {
    }
    ...
    GpxType convertFromOld(Gpx gpx){
        ...
        newGpx.setWpt(createWaypoints(gpx.getWpt()));
        newGpx.setRte(createRoutes(gpx.getRte()));
        newGpx.setTrk(createTracks(gpx.getTrk()));
        return newGpx;
    }

    List<TrkType> createTracks(List<Gpx.Trk> trks){
        List<TrkType> tracks = new ArrayList<TrkType>();
    ...
    TrksegType createTrackSeg(Gpx.Trk.Trkseg trkseg){
        ...
        List<WptType> trkpts = new ArrayList<WptType>();
        for(Gpx.Trk.Trkseg.Trkpt trkpt:trkseg.getTrkpt()){
            trkpts.add(convertOldTrkPntToWpt(trkpt));
        }
        newTrkseg.setTrkpt(trkpts);

        return newTrkseg;

    }

    List<RteType> createRoutes(List<Gpx.Rte> routes){
        List<RteType> newRoutes = new ArrayList<RteType>();
